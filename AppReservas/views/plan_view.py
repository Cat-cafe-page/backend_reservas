...
#Get Plans depending on the daytime 
...
#Get Plans depending on the duration 
class PlanForDurationDetailView(generics.ListAPIView):
    queryset            = Plan_usuario.objects.all()
    serializer_class    = PlanSerializer
    def get_queryset(self):
        queryset = Plan_usuario.objects.filter(duracion=self.kwargs["duracion"])
        return queryset
